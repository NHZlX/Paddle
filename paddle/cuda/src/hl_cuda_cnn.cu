...
__global__ void upsampleForwardCompute(real *input_data, real *mask_data,
                                      size_t nthreads,
                                      size_t in_h,
                                      size_t in_w,
                                      size_t out_h,
...
__global__ void upsampleBackwardCompute(real *out_grad, real *mask_data,
                                       size_t nthreads,
                                       size_t in_h,
                                       size_t in_w,
                                       size_t out_h,
  ...
  if (index) {
    int offset = index / (in_w * in_h) * out_h * out_w; 
    int upsample_idx = static_cast<int>mask_data[index];
    input_grad[index] = top_diff[offset + upsample_idx];
  }
}

void hl_upsample_forward(real *inputData, real *maskData,
                        size_t batchSize,
                        size_t imgSizeH,
                        size_t imgSizeW,
                        size_t channels,
  ...
  int blocks = (num_kernels + 1024 - 1) / 1024;
  upsampleForwardCompute<<<blocks, 1024, 0, STREAM_DEFAULT>>>(
    inputData, maskData, num_kernels, imgSizeH, imgSizeW, outputH, outputW, outputData);
  CHECK_SYNC("hl_upsample_forward failed");
}

void hl_upsample_backward(real *outputGradData, real *maskData,
                        size_t batchSize,
                        size_t imgSizeH,
                        size_t imgSizeW,
                        size_t channels,
  ...
  int blocks = (num_kernels + 1024 - 1) / 1024;
  upsampleForwardCompute<<<blocks, 1024, 0, STREAM_DEFAULT>>>(
    outputGradData, maskData, num_kernels, imgSizeH, imgSizeW, outputH, outputW, inputGradData);
