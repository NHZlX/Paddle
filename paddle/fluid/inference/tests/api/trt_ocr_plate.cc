...
std::string fluid_predict() {
	...
	//	static_cast<float*>(image_tensor.data.data())[c++] = val;
	//}
    //init ids
    ...
    fluid_predictor->Run(input_tensors, &output_tensors, 1);
    auto time1 = time(); 
    for(int i = 0; i < 1000; i++)  {
      fluid_predictor->Run(input_tensors, &output_tensors, 1);
    }
    auto time2 = time(); 
    std::cout <<"batch: " << 1 << " predict cost: " << time_diff(time1, time2) / 1000.0 << "ms" << std::endl;
...
int main(int argc, char** argv) {
    ...
    MixedRTConfig paddle_config;
    float fraction_of_gpu_memory = 0.1f;
    bool use_gpu = true;
    ...
    paddle_config.minimum_subgraph_size=3;
    paddle_config.device = 0;
    paddle_config.specify_input_name = true;

    fluid_predictor =
      paddle::CreatePaddlePredictor<MixedRTConfig, paddle::PaddleEngineKind::kAutoMixedTensorRT>(paddle_config);
