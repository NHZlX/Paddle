 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
     ...
     */
      ...
      auto *output_temp_data = fluid_t->mutable_data<float>(platform::CPUPlace());
      for (size_t i = 0; i < fluid_t->memory_size()/ sizeof(float); i++) {
	std::cout << output_temp_data[i] << " ";
      }
      std::cout << std::endl;
